...
export default function DocsSelector({ onClose }: DocsSelectorProps) {
  // const router = useRouter(); // 不再需要 useRouter
  const { docs, openDocs, openDoc } = useDocs(); // 移除 workspaceType
  const [searchQuery, setSearchQuery] = useState("");

  const filteredDocs = docs.filter(
    (doc) =>
      doc.type === "document" && // 只显示文档，不显示文件夹
          ...
          <div className="flex-1 overflow-y-auto p-4">
            <div className="space-y-1">
              {filteredDocs.map((doc) => {
                const isOpen = openDocs.some((d) => d.uid === doc.uid);

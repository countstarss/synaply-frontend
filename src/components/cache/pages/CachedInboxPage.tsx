...
export const CachedInboxPage = React.memo(() => {
  ...
  const pageState = getPageState("inbox");

  console.log("📧 CachedInboxPage渲染:", {
    pageState,
    hasData: !!pageState?.data,
    cachedMessages: pageState?.data?.messages,
  });

  // 从缓存中获取状态，或使用默认值
  const [messages, setMessages] = useState<Message[]>(
    (pageState?.data?.messages as Message[]) || MOCK_MESSAGES
  );

  // 更新缓存数据
  useEffect(() => {
    updatePageData("inbox", {
      messages,
    });
  }, [messages, updatePageData]);

  const toggleStarred = React.useCallback((messageId: string) => {
    setMessages((prev) =>
      prev.map((msg) =>
